		...
		if (!isset($options["rename"]) || !$options["rename"])  $result = array("success" => false, "error" => self::FETranslate("Operation denied."), "errorcode" => "rename_not_allowed");
			...
			if ($result === false) {
				if (file_exists($path . "/" . $newname)) {
					// Jika file dengan nama baru sudah ada, tambahkan suffix untuk menghindari overwrite
					$suffix = 1;
					$filename_parts = pathinfo($newname);
					$filename = $filename_parts['filename'];
					$extension = isset($filename_parts['extension']) ? "." . $filename_parts['extension'] : "";

					while (file_exists($path . "/" . $filename . " (" . $suffix . ")" . $extension)) {
						$suffix++;
					}

					$newname = $filename . " (" . $suffix . ")" . $extension;
					// Cobalah rename lagi dengan nama yang sudah diperbarui
					if (@rename($path . "/" . $file, $path . "/" . $newname) !== false) {
						$entry = self::BuildEntry($path, $newname, (is_dir($path . "/" . $newname) ? "folder" : "file"), $depth, $options);
						$result = array(
							"success" => true,
							"entry" => $entry
						);
					} else {
						$result = array("success" => false, "error" => self::FETranslate("Rename operation failed."), "errorcode" => "rename_failed");
					}
				} else
		...
		if (!isset($options["file_info"]) || !$options["file_info"])  $result = array("success" => false, "error" => self::FETranslate("Operation denied."), "errorcode" => "file_info_not_allowed");
			...
			if (!self::HasAllowedExt($allowedexts, $options["allow_empty_ext"], $name))  $result = array("success" => false, "error" => self::FETranslate("The file extension is not allowed."), "errorcode" => "invalid_file_ext");
				...
				if (substr($data, 0, 6) === "GIF87a" || substr($data, 0, 6) === "GIF89a") {
					$mimetype = "image/gif";
					$browser = true;
				} else if (substr($data, 0, 8) === "\x89PNG\x0D\x0A\x1A\x0A") {
					$mimetype = "image/png";
					$browser = true;
				} else if (substr($data, 0, 2) === "\xFF\xD8" || (!$text && ($ext === "jpg" || $ext === "jpeg"))) {
					$mimetype = "image/jpeg";
					$browser = true;
				} else if ($text && $ext === "svg") {
					$mimetype = "image/svg";
					$browser = true;
				} else if (substr($data, 0, 3) === "ID3" || (substr($data, 0, 1) === "\xFF" && ord(substr($data, 1, 1)) >= 0xE0) || (!$text && $ext === "mp3")) {
					$mimetype = "audio/mp3";
					$browser = true;
				} else if (substr($data, 4, 8) === "ftypmp42" || substr($data, 4, 8) === "ftypMSNV" || substr($data, 4, 8) === "ftypisom" || (!$text && $ext === "mp4")) {
					$mimetype = "video/mp4";
					$browser = true;
				} else if ($text && (stripos($data, "<!DOCTYPE") !== false || stripos($data, "<html") !== false || $ext === "html" || $ext === "htm")) {
					$mimetype = "text/html";
					$browser = true;
